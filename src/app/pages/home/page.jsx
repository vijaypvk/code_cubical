...
export default function Home() {
  ...
  return (
    <div className="relative flex flex-col lg:flex-row min-h-screen bg-black overflow-hidden ">
      ...
      <button
        onClick={() => setSidebarOpen(!sidebarOpen)}
        className="absolute top-4 left-4 z-50 lg:hidden bg-zinc-800 p-2 rounded-md text-white hover:bg-zinc-700 transition-colors"
      >
        <Menu size={20} />
      </button>
      ...
      {/* Sidebar */}
      <div
        className={`lg:relative lg:block fixed inset-y-0 left-0 z-40 w-64 transition-transform duration-300 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0 bg-zinc-900`}
      >
        <Sidebar isOpen={sidebarOpen} />
      </div>
      ...
      <main className="flex-1 relative">
        ...
        <main className="relative flex-1 flex items-center justify-center overflow-hidden min-h-screen bg-black text-white">
          ...
          <div
            className="absolute 
      bottom-5 left-1/2 transform -translate-x-1/2 
      md:left-[270px] md:translate-x-0 
      w-full max-w-6xl px-4 z-50"
          >
